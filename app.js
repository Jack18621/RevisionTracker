...
const KEY = 'revise:web:flat:v2';
const DEFAULTS = {
  dailyTarget:45, includeWeekends:true,
  ...
  homework:[],
  proxyUrl:'', studentCode:'', studentDob:'',
  examDates:[
...
// ====== quotes ======
const QUOTES = [
  "Small steps daily beat rare sprints.",
  "Progress > perfection.",
  "You only lose when you stop.",
  "Show up. Good things follow.",
  "Sharpen the axe a little every day.",
  "Tiny wins compound into big ones.",
  "Future-you says thanks.",
  "Consistency makes smart look genius.",
  "1% better today.",
  "Trust the process.",
  "Focus for 25, rest for 5.",
  "Hard things are practice for harder things.",
  "Discipline is a shortcut.",
  "You can't edit a blank page.",
  "Your only job is to start."
];
const quoteForDay = (d) => QUOTES[(Math.abs(d.getFullYear()*37 + d.getMonth()*13 + d.getDate()) % QUOTES.length)];
...
// ====== subjects (for focus only) ======
function fillSubjects(){
  const b = el('subjectSelectFocus'); b.innerHTML='';
  for(const s of DATA.subjects){ b.add(new Option(s.name,s.id)); }
}

// ====== sessions/timer (focus in HOME) ======
let tick=null, elapsed=0, running=false;
function updateControls(){
  el('focusStart').classList.toggle('hide', running);
  el('focusPause').classList.toggle('hide', !running);
  el('focusStop').classList.toggle('hide', !running);
  el('ringWrap').classList.toggle('pulse', running);
}
function startSession(subjectId){
  stopSession(); running=true; updateControls();
...
function stopSession(){
  ...
  running=false; updateControls(); save(); drawChart(); refreshHistory(); renderStreak(); updateQuote();
}
...
function computeStreak(){
  const includeW = DATA.includeWeekends;
...
function renderStreak(){
  const grid = el('streakGrid'); grid.innerHTML='';
  const start = new Date(DATA.rangeStart), end = new Date(DATA.rangeEnd);
  const per = minutesPerDay(start,end);
  const includeW = DATA.includeWeekends;
  const target = DATA.dailyTarget;
  const days = [];
  for(let i=29;i>=0;i--){ const d = new Date(); d.setDate(d.getDate()-i); days.push(d); }
  days.forEach(d => {
    const k = d.toISOString().slice(0,10);
    const mins = per[k] ? Object.values(per[k]).reduce((a,b)=>a+b,0) : 0;
    const ok = (includeW || (d.getDay()!=0 && d.getDay()!=6)) && mins>=target;
    const cell = document.createElement('div'); cell.className = 'cell' + (ok?' ok':'');
    cell.title = `${k} • ${Math.round(mins)} min\n${quoteForDay(d)}`;
    grid.appendChild(cell);
  });
  el('streakLabel').textContent = 'Streak: '+computeStreak()+' 🔥';
}
function updateQuote(){ el('quoteBox').textContent = '“'+quoteForDay(new Date())+'”'; }

// ====== chart (no subject dropdown here) ======
let chart;
function drawChart(){
...
function bindSettings(){
  ...
  el('dailyTarget').onchange = ()=>{ DATA.dailyTarget=+el('dailyTarget').value||45; save(); renderStreak(); drawChart(); };
  el('includeWeekends').onchange = ()=>{ DATA.includeWeekends=el('includeWeekends').checked; save(); renderStreak(); };
  el('rangeStart').onchange = ()=>{ DATA.rangeStart=el('rangeStart').value; save(); drawChart(); renderStreak(); };
  el('rangeEnd').onchange = ()=>{ DATA.rangeEnd=el('rangeEnd').value; save(); drawChart(); renderStreak(); };
...
// ====== boot ======
function boot(){
  fillSubjects(); bindSettings(); drawChart(); refreshHistory(); renderStreak(); updateQuote(); updateControls();
}
boot();
