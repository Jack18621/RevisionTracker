...
(function(){
  ...
  function loadManifest(){
    fetch("./config/manifest.json").then(function(r){ return r.json(); }).then(function(m){
      state.manifest = m;
      populatePapers();
    });
  }
  ...
  function populatePapers(){
    var sel = $("#paperSel");
    sel.innerHTML = state.manifest.papers.map(function(p){ return '<option value="'+p.id+'">'+p.name+'</option>'; }).join("");
  ...
  function render(){
    ...
    if(state.mode==="MATCH"){
      ...
      left.querySelectorAll(".dropzone").forEach(function(zone){
        ...
        zone.addEventListener("drop", function(e){ e.preventDefault(); var def=e.dataTransfer.getData("text/plain"); zone.textContent = zone.dataset.key + " — " + def; zone.dataset.choice = def; });
      });
